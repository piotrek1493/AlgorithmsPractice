...
public class BuyingACar {
    public static int[] nbMonths(int startPriceOld, int startPriceNew, int savingperMonth, double percentLossByMonth) {
        ...
        while(startOld + savings < startNew) {
            months++;
            if(months > 0 && months % 2 == 0) {
                percentLossByMonth += 0.5;
            }
            startOld *= (100-percentLossByMonth)/100;
            startNew *= (100-percentLossByMonth)/100;
            savings += savingperMonth;
        }

        return new int[]{months, (int)Math.round(startOld + savings - startNew)};
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(nbMonths(42000, 127000, 3500, 2)));
    }
}
